...
Name: Farooq Haider
\\[3ex]
Signature: \includegraphics[height=1.25cm]{Figures/sign1.jpg} 
\\[6ex]
Name: Abdullah Azeem
\\[3ex]
Signature: \includegraphics[height=1.25cm]{Figures/sign2.jpg}
\\[6ex]
Name: Hasnain Khurram
\\[3ex]
Signature: \includegraphics[height=1.25cm]{Figures/sign3.jpg} 
...
The main focus of this project is to research the datasets, models and limitations of existing assistive technologies for visually impaired individuals to develop an AI-based navigation system that overcomes the limitations of existing solutions. Our fine-tuned model will be enclosed in a thoughtfully designed React Native mobile application, specific to the needs of the visually impaired individuals, which will make use of the smartphone camera to detect obstacles in real-time and guide users on safe paths via audio or haptic feedback. Through this project, We aim to overcome limitations of existing technologies that require, for example, expensive hardware like LiDAR, ultrasonic sensors, mixed-reality headsets or human volunteers thus sacrificing autonomy, affordability, and scalability. 
...
\section{Problem Statement}
 Visually impaired people frequently encounter safety and mobility challenges indoors: static furniture, low
hanging obstacles, dynamic obstacles (people, carts), and clutter that a white cane or guide dog may miss
 at head or torso level. Existing approaches either rely on mapped environments, additional hardware,
 volunteer assistance, or provide only object identification and distance, not continuous, actionable
 navigation guidance tailored for indoor walking. The problem addressed here is: \textbf{How to provide fast,
 reliable, and accessible indoor obstacle detection and guidance using a mobile device camera and
 sensors, so visually impaired users can navigate common indoor settings independently and safely.}
\section{Problem Elaboration}
 The navigation problem for visually impaired users decomposes into several sub-problems:
\begin{itemize}
    \item \textbf{Real-time Obstacle Detection:} Detect common obstacles (chairs, tables, doors, stairs, people,
 boxes) from a monocular camera stream at walking speed. Detection must work under variable
 ...
 obstacles (static and moving) and prioritize which obstacles present immediate collision risk.
    \item \textbf{Direction and Path Guidance:} Convert detection and distance into short, clear guidance that indicates
 a safe direction to move.
 indoor lighting and clutter.
    \item \textbf{Feedback Design (Audio and Haptic):} Create a feedback layer with concise text to speech prompts, and patterned haptic cues for immediate reactions.
    \item \textbf{Performance and Resource Constraints:} Allow the application to
 run on an average mobile device by keeping low CPU/GPU, memory, and battery usage.
 
    \item \textbf{Usability and Safety Testing:} Conduct testing with end users to
 minimize confusion in interpreting feedback cues. Build safety fallbacks such as immediate stop cue on sudden
 appearnce of a highly close object.
\end{itemize}
\section{Goals and Objectives}
Through Sighteo, we aim to make navigation easier and affordable for visually impaired individuals. This section covers a breakdown of the goals and objectives we aim to accomplish through this project.
\begin{itemize}
    \item Deliver a reliable mobile-based indoor navigation aid that reduces collisions and improves
 independent mobility for visually impaired users.
    \item Optimize the system for indoor scenarios so detection and guidance are accurate and low-latency.
    \item Implement and evaluate an on-device object detection pipeline that achieves real-time frame rates
 ...
 directional instructions.
    ...
    \item Test the system in representative indoor environments (classrooms, corridors, shopping aisles, living
 rooms) and measure collisions, false positives, latency, and user satisfaction.
\end{itemize}
\section{Project Scope}
The project intends to create a mobile application for indoor navigation help aimed at visually impaired users in design, implementation, and evaluation. The application will:

\begin{itemize}
    \item The primary input for the system will be the mobile's camera feed.
    \item Real-time object detection and proximity estimation will be done, and the results will be translated into audio and haptic guidance.
    \item Support only indoor environments (offices, homes, malls, corridors). Outdoor navigation, GPS-based
 routing, public-transport integration, and pre-mapped building routing are out of scope.
\end{itemize}
This narrower scope allows focused optimization for indoor-specific challenges (lighting, clutter, head/waist
...
Our project that merges AI-driven object recognition with real-time feedback. This will contribute to making cities and communities that are more inclusive, resilient, and supportive of people with disabilities. This comes in harmony with the global mission of SDG 11.
...
\section{Constraints}
Although promising, Sighteo still has some limitations. Those limitations are listed below:

\begin{itemize}
    \item \textbf{Environmental Factors}: Low-light conditions, crowded environments, and presence of fast moving obstacles may cause variance in detection accuracy.
    ...
    \item \textbf{Testing Environment}: Initial deployment will focus on controlled or semi-controlled indoor spaces.  This may be expanded to testing in complex outdoor environments in the future.
    \item \textbf{User-Centered Design Issues:}: Designing for people with visual impairments fundamentally relies on iterative feedback and usability testing with real users. That does not always become easily possible.
...
\begin{itemize}
    \item \textbf{Primary Users}: Visually impaired individuals trying to seek safe, independent navigation.
    \item \textbf{Secondary Users}: Caregivers, and families having visually impaired individuals.
    \item \textbf{Project Team}: Developers/AI Engineers responsible for tasks like model training, deployment, and application design. Testers for validating the use of the application.
\end{itemize}

\subsection{Key High-Level Goals and Problems of Stakeholders}
\begin{itemize}
    \item \textbf{Visually Impaired Users}: Need a reliable real-time navigation support for reducing dependency on others. Their main challenge is safety. They will expect accuracy from the system to trust it.
    ...
    \item \textbf{Developers/Researchers}: Aim to deliver a robust on-device system. The system must be capable of handling real-time object detection. Their challenge is balancing accuracy, latency, and connectivity requirements.
\end{itemize}

\section{Conclusion}
This chapter introduced the motivation, scope, and direction of the Sighteo project: defining the problem that people with visual impairments face and framing a solution with respect to low cost, accessibility, and real-time indoor navigation. The problem domain was established, the objectives of the system were explained, and its societal impact was underlined in relation to global development goals. By identifying stakeholders, constraints, and opportunities, this chapter gives clear ground to the technical decisions and design strategies presented later in the text.


\chapter{Literature Review / Related Work}
This chapter of our report documents past literary works and existing technologies in the domain of assistive navigation for visually impaired individuals. Through this literature review, we aim to identify trends, strengths, and limitations of existing methods and systems. Academic researches were analyzed alongside existing commercial applications to provide a view of the landscape of this project. For each literary work or application, there is a summary, critically analysis comparison with Sighteo. The chapter concludes with summary tables that precisely mentions the findings from each work discussed.
...
This section of the report documents the review of literature consisting of past researches, studies, and existing competing solutions related to assistive navigation for visually impaired individuals. It discusses studies that introduce smartphone-based systems, object recognition applications, and real-time pathfinding algorithms, analyzing their strengths and limitations. By examining prior research and market alternatives, this review establishes the foundation for our proposed system, identifies critical gaps, and positions our work as a step forward in AI-powered navigation assistance.

\subsection{A Smartphone-Based Mobility Assistant Using Depth Imaging for Visually Impaired and Blind}
This work is one of the first efforts to investigate how smartphone depth sensors can support the mobility of visually impaired and blind users. This study focuses on demonstrating that depth information captured directly from a phone will be able to detect nearby obstacles without using any external devices. By leveraging the integrated sensing capabilities of modern smartphones, this system can offer basic spatial awareness and timely obstacle alerts. On the whole, the work illustrates that smartphone-based depth imaging is effective for serving as a practical basis in assistive navigation.

\subsubsection{Summary of the Article}  
See, Sasing, and Advincula \cite{ref:see2022smartphone} introduced a smartphone-based mobility assistant for visually impaired and blind individuals that utilizes depth imaging technology. The core of their system lies in leveraging the built-in depth cameras of modern smartphones to provide users with spatial awareness and obstacle detection without the need for additional hardware. The technology extracts depth data from 23 coordinate points that are located within the scene that is being captured and then it divides the obstacles into the prescribed zones: head zone, torso zone, ground zone, or full body obstacles. The maximum range for obstacle detection mentioned was 1.6 meters which is the range suitable for being aware of the objects around.
...
The most intuitive way of providing feedback is revealed by the new methodology of categorizing hindrances as particular body zones. The limitations of See et al.’s \cite{ref:see2022smartphone} system highlight the areas that our project aims to address. Their work is meant for indoor and outdoor settings, but it fails to provide the needed assistance particularly in indoor environments where the challenges are more serious due to narrow hallways, irregular furniture arrangements, and cluttered obstacles. By contrast, our project focuses exclusively on indoor spaces. Unlike See et al.’s work, which primarily alerts users to obstacles, our proposed system aims to provide real-time guidance by indicating safer pathways and suggesting navigation directions. This transforms the system from a passive warning tool into an active guide. Additionally, while their system uses a fixed set of coordinate points for detection, our project intends to employ more comprehensive real-time object detection techniques to improve accuracy in identifying both static and dynamic obstacles. Lastly, See et al. \cite{ref:see2022smartphone} set a good platform by proving that depth imaging on smartphones can benefit visually impaired users. But the restricted range of detection, lack of directional cues, utilization of fixed points, and focus on multimodal control all indicate vasts areas of improvement. Our research builds on these observations in the direct sense of being centered on indoor spaces and providing better navigation assistance.

\subsection{Android-Based Object Recognition Application for Visually Impaired Users}
This work presents a simple yet meaningful attempt to develop an Android-based object recognition app supporting the needs of visually impaired users. The authors focus on showing how lightweight models can be deployed directly on the mobile device, independent of cloud services, to make assistive tools more accessible and affordable. By combining real-time object detection with a speaking interface, this study demonstrates a practical method for audio cues about environmental understanding by users. It demonstrates how on-device AI can improve everyday independence in visually impaired users.
...
In this paper, the authors show a practical method for implementing object detection in a mobile app that has offline functionalities. Using the SSD architecture is a great decision for resource-limited environments. The choice of Tensorflow Lite is an indicator that the developers are aware of the issues related to the practical deployment of mobile applications. However, the limitations of the system in terms of design, evaluation, and claims become clear on careful examination. Firstly, the 90\% accuracy they claim is not only about false positives, latency, or performance in difficult indoor conditions like low light, occlusion, or clutter. It mainly concerns either static scenes. Secondly, the system only informs the user about the names of the objects only. It does not offer any guidance for navigation. Thirdly, the system ignores dynamic obstacles such as moving people. Fourthly, there is not any mention of real-time performance such as frames per second or latency, or energy consumption, which are all critical for any assistive tool. In addition, the ability to read texts is a nice feature to have, but the authors fail to mention how dependable this is when lighting changes, the text is placed at an angle or on a curved surface, and how the user experience is impacted by switching between recognition of objects and reading of texts. The usability of the system under walking real-life conditions in rooms where a lot of objects or surfaces are mixed is not completely tested and reported.

\subsubsection{Relationship to the Proposed Research Work}  
Bhagway et al.’s \cite{ref:salunkhe2021android} work offers a useful reference point and a technological baseline. It demonstrates the feasibility of running object detection and voice feedback in real time on a mobile device. It establishes the fact that mobile phones can handle object recognition without relying on server infrastructure. However, the gaps in their work align closely with the problems Sighteo aims to solve. Their system stops at object identification and vocal naming. It does not go further into active navigation guidance. And, that is central to our goal. Because Bhagway et al. do not consider indoor specific constraints such as tight hallways, clutter, floor level hazards, or moving obstacles, their design is generic. Sighteo will build on this foundation. Through Sighteo, we aim to enhance performance providing continuous directional guidance. We will also focus on minimising latency, which was not the focus in this paper. After establishing that object recognition on a phone for the visually impaired is indeed possible, the paper leaves an open challenge of converting that recognition into safe real time navigation.


\subsection{Efficient Real-Time Pathfinding for Visually Impaired Individuals}
This work redirects focus from mere obstacle detection to the computation of safe real-time paths for visually impaired users. Instead of merely attaining awareness, it investigates how algorithms can lead users through dynamic scenes by computing collision-free paths as they move. The work emphasizes low latency, adaptability, and computational efficiency, demonstrating that effective pathfinding forms a backbone on which to base live navigation support. Overall, it shows how computation of real-time routes can enhance mobility beyond object detection alone.

\subsubsection{Summary of Research Item}  
Ghahremanians and Mohammadi \cite{ref:ghahremanians2025efficient} present an approach to efficient real-time pathfinding designed for visually impaired individuals. Their work focuses on computing safe, collision-free paths as users move through environments with obstacles and possibly dynamic elements. The proposed method aims to reconcile several challenges: low latency, computational efficiency, and adaptability to changing surroundings. The algorithm is designed for real-time execution and therefore more applicable to live guidance, but less so for offline route planning. Despite the paper's focus being generic navigation, its path planning techniques in constrained environments are more applicable to assistive systems.

\subsubsection{Critical Analysis of the Research Item}  
This work is worthwhile since it addresses the essential problem translating obstacle information into navigable routes, instead of merely detecting objects. Previous systems often get only up to the point of "alert user to object," while in Sighteo the problem solved is determining what path the person should move along in order to prevent collisions. Stressing real-time capability suggests the authors are aware of the importance of responsiveness, which in a navigation aid can be paramount. Negative aspects are that the publicly available summaries are uninformative about precise evaluation outcomes, particularly under indoor, cluttered, or dynamic conditions. It's also indeterminate how uncertain or noise-laden input (e.g. from a moving-camera phone's camera) is handled. Another drawback is whether the technique presumes a mapped environment (or pre-known layout), versus operating in arbitrary, unseen spaces. Furthermore, the paper apparently targets quite widely (for general visually impaired navigation); we are not clear that it's particularly optimized for indoor limitations such as narrow hallways, objects at the person's level, or overlap furniture. The computational efficiency claims are promising, but we are uninformed about memory usage, energy usage, or how well it tolerates latency in low-spec devices (e.g. phones). Lastly, dealing with dynamic objects (moving people or moving objects) represents a serious practical problem, and it's indeterminate from the summary how stable their method is in this respect.

\subsubsection{Relationship to the Proposed Research Work}  
This research in specific is up-to-date since it fills the gap in the literature from obstacle detection up to path planning. For our potential app, their research illuminates methodically how to compute in real time safe paths from sensor readings. It demonstrates that pathfinding can be a substantial part of a support for navigation aid and not merely a theoretical construct for research's sake. Meanwhile, our solution possesses a great number of features which are specific only to ours. We consider only indoor navigation with all its specific characteristics like restricted space, clutter, varying illumination, moving objects, shadowing effects, stairs, and narrow corridors. The approach described by Ghahremanians and Mohammadi \cite{ref:ghahremanians2025efficient} does not limit its applicability for indoor-only use and thus potentially overlooks some indoor-specific issues. Additionally, our application is intended to provide unobstructed guidance (directions, avoidance maneuvers) during user movement, perfectly synchronizing with user detection rather than relying on prior mapping or grand outdoor-style planning. Their approach is based on efficiency; thus, we are able to employ their strategies for latency reduction or simplified path planning but we need to make sure that they are adapted to effectively work in the challenging situations created by noisy, occluded, and rapidly changing indoor mobile camera input. In short, their research informs the path planning component of our system, but we must extend it with indoor specialization, tighter integration with object detection, and user-centric feedback (audio + haptic) to make it work in practice in indoor walking assistance.


\subsection{Improved YOLOv5 Algorithm Combined with Depth Camera and Embedded System for Blind Indoor Visual Assistance}
This work investigates the improvement of YOLOv5 by fusing depth camera data, optimized embedded hardware, and refinements in the model architecture for indoor assistance applications. The authors investigate how depth information may strengthen detection performance, especially when the scenes are cluttered or objects are very close to the camera. The paper also underlines how architectural adjustments will lower the computational load without performance degradation and make the system more apt for real-time processing on lightweight devices used indoors.
...
The following work provides a general overview of assistive navigation technologies developed for visually impaired users. In this work, a wide range of existing solutions is reviewed; also, there is a categorized overview by sensing methods, system architectures, and techniques for user interaction. This paper maps out how different sensors and feedback approaches are used across current systems, giving a clearer understanding of the technological landscape. This broader perspective brings to light not only the strengths of the available methods but also the points that still require attention in future designs.

\subsubsection{Summary of the Research Item}  
Abidi et al. \cite{ref:abidi2024comprehensive}, in this paper, reviewed works by scientists who experimented with a range of devices to assist the visually impaired. These include wearable such as haptic vests and smart glasses, and non-wearable ones such as navigation sticks and sensory systems. Both the RGB-D cameras and tactile sensors can be utilized so that the user can have a greater understanding of his/her surroundings, whereas IoT-powered solutions and satellite guidance allow more users to access the system simultaneously \cite{ref:abidi2024comprehensive}. Furthermore, the study reveals that the human centered design approach is the crucial factor for the usability and customization and thus it will resultin an increase in the quality of life of the blind and visually impaired individuals \cite{ref:abidi2024comprehensive}.

\subsubsection{Critical Analysis of the Research Item}  
One of the best strengths of this piece is its thorough categorization of current solutions and discourse of their technical underpinnings, which provides a strong foundation for future research. However, thepaper largely canvasses existing technologies without critically confronting the limitations of real-worlddeployment, including internet access, expense, and obstacles to user adoption. Such gaps mean thatwhereas the review is complete, it is more descriptive than evaluative regarding practical application.

\subsubsection{Relationship to the Proposed Research Work}  
For our proposed work, this review is a fine benchmark by identifying the importance of integrating artificial intelligence and mobile-based solutions for visually impaired navigation. Since our project is designed to apply a smartphone camera for displaying real-time navigation directions, the outcomes directly support the relevance and feasibility of our approach while also reinforcing the need to address practical constraints like connectivity reliability, which are hardly addressed in the review \cite{ref:abidi2024comprehensive}.

\subsection{Assistive Systems for Visually Impaired Persons: Challenges and Opportunities}
This review covers the key challenges intrinsic to the design of effective assistive systems and emphasizes how new sensing technologies and AI methods offer possibilities for their improvement. It underlines issues such as accuracy, reliability, user comfort, and real-world functioning. It also discusses opportunities opened up by both advancements in mobile hardware and lightweight machine learning models. In general, this review outlines core factors that will eventually shape usability and influence whether such technologies are likely to be adopted by visually impaired users.

\subsubsection{Summary of the Research Item}  
Okolo et al. \cite{ref:okolo2024assistive} offer a review targeting the challenges and opportunities in developing assistive systems for the visually impaired, with specific interest in navigation aids. The paper categorizes assistive technologies into sensing inputs (e.g., smartphones, RFID, depth cameras, infrared, ultrasonic, RGB-D), feedback mechanisms (audio, tactile, vibration, haptic), and IoT-based systems. It highlights the need for the incorporation of artificial intelligence, computer vision, and mobile technologies to facilitate the improvement in obstacle detection, spatial understanding, and real-time navigation assistance. The review also covers both commercially used tools (e.g., SmartCane, WeWalk) and research prototypes in order to get a wide view of existing technology developments.

\subsubsection{Critical Analysis of the Research Item}  
One of the strengths of this work lies in its in-depth categorization of technologies and comparative analysis of AI- and IoT-based solutions. It extends merely describing tools by including limitations such as bulkiness, expense, low performance in well lit conditions, and difficulties in user uptake. Nevertheless, the review mainly exists in the realm of theory and does lack empirical analysis of system usability in real-world settings. Furthermore, whereas it lays out future suggestions such as multi-modal feedback and mobility, the absence of concrete design guidelines or user-centric confirmation stands out as a significant omission.

\subsubsection{Relationship to the Proposed Research Work}  
This paper also links well with our project idea because it points out the necessity for mobile and AI-based navigation systems. Since our system relies on a smartphone camera for obstacle detection and directions, the result in terms of limitations in vision-based systems (e.g., ineffectiveness in depth estimation in normal cameras) directly corresponds with the expected technical challenges we must transcend. Furthermore, the recommendation for light, userfriendly, and portable systems corresponds well with our goal of developing a feasible, smartphone-based navigation system specifically with the visually impaired in view \cite{ref:okolo2024assistive}.


\subsection{Technological Advancements in Human Navigation for the Visually Impaired: A Systematic Review}
This systematic review details recent advances in navigation technologies designed for the visually impaired, putting current development into a larger technological and practical context. This review reveals emergent trends in sensing, localization, and feedback mechanisms, while also mapping the common design patterns that many systems adhere to. Simultaneously, this review identifies key gaps that still hold back widespread adoption due to usability, cost, reliability, or real-world performance issues. These insights, taken together, help clarify where future research and development should be focused.
...
\subsubsection{Critical Analysis of the Research Item} 
Among its key strengths, the review stands with its systematic and exhaustive coverage, employing PR
the
Accept
Dismiss
ISMA methodology (a protocol for enhancing the clarity and quality of reports of systematic reviews and meta-analyses) for ensuring rigor. It classifies solutions into smartphone-based systems, haptic interfaces, and AI-based naviagation aids and reports that 79\% of the studied surveys were included with experimental validation, with haptic feedback having the highest prevalence as a modality. The review also pinpoints the weaknesses in existing systems, including dependency on GPS, high expense, and low uptake among users due to usability issues. The weaknesses suggest the directions towards more affordable, accessible, and user-oriented designs. 

\subsubsection{Relationship to the Proposed Research Work}  
This project directly substantiates our project's mission. It validates our decision to combine AI-powered object detection with audio and haptic feedback and sheds light on the gaps we aim to close, notably improving indoor navigation and designing with affordability and access in mind. We are building on the literature's strengths and aiming to avoid the weaknesses in order to create a useful solution.
...
\subsubsection{Summary of the Research Item}  
Patel et al. \cite{ref:patel2025enhancing} give an overview of machine learning methodologies used to aid visually and hearing-impaired people, including classic ML and deep learning. For hearing, they give an overview of models like SVM, Random Forests, and MLPs for speech and sound classification tasks. For vision, they offer a rundown of real-time object detection models like YOLO, SSD, and RetinaNet that enable scene understanding and navigational help on wearables and smartphones. The article also cites Generative AI applications and points out how fine-tuning and personalization are typically needed to fit user needs. Generally, the paper follows the trajectory from device, dataset, and algorithm to referring to the ways in which these approaches become useful tools for assistance.

\subsubsection{Critical Analysis of the Research Item}  
Strength lies in the fact that the paper covers two sensory modalities and clearly separates method by task and model family, making it a good place to start design selection. It is also concerned with real-time constraints and the importance of model-specific user tuning, which brings one close to real-world in deployment. Being a general survey, it does not include head-to-head comparisons under a common protocol, and therefore comparative performance has to be deduced from a number of studies. On-device limitations (battery, thermal throttling) and user study discussion are minimal, and there are unresolved issues of long-term acceptability and cost in field use. These are reasonable trade-offs for a survey but have to be recognized when progressing insights to an operational system.

\subsubsection{Relationship to the Proposed Research Work}  
For Sighteo's indoor navigation emphasis, the paper reaffirms employing contemporary object detectors (e.g., YOLO/SSD/RetinaNet) and selecting an appropriate dataset to find accuracy vs. speed balance. It also encourages us to think through personalization and feedback modalities upfront, as most assistive solutions take advantage of user-adjusted thresholds and interfaces. Practically, this review helps us shortlist model families for experimentation, set evaluation criteria around latency and reliability, and plan for future extensions (for example, exploring lightweight Generative AI features for descriptions or captions once core navigation is stable).
...
A framework of deep learning and IoT-based indoor activity monitoring is introduced in this literature. Although the work is not focused on navigation, it still provides insight into an ensemble model, sensor fusion, to improve the safety and situational awareness of a visually impaired user. This perspective broadens our view of how multiple streams can work in tandem to create more reliable assistive environments.

\subsubsection{Summary of the Research Item}

Al Duhayyim \cite{ref:al2025ensemble} proposed an ensemble-based deep learning and Internet of Things (IoT) framework known as EDLES-SIAM (Ensemble of Deep Learning for Enhanced Safety in Smart Indoor Activity Monitoring) to enhance safety and independence for visually impaired individuals in indoor environments. The system combines multiple deep learning architectures, including Deep Neural Networks (DNN), Bidirectional Long Short-Term Memory (BiLSTM), and Sparse Stacked Autoencoders (SSAE), into a unified ensemble to perform accurate human activity recognition (HAR). The approach starts with Adaptive Bilateral Filtering (ABF) to filter sensor and image noise and then feature extraction is done using ResNet50 for spatial feature extraction. The ensemble outputs are summed up sequentially to infer human activity as well as abnormal activity like falls or unsafe movement. UR Fall Detection dataset was experimented on test and classification accuracy because of it 99.25\% in comparison to standalone deep models such as VGG16, VGG19, and certain ResNet versions. The paper discusses how combined IoT and deep learning systems individually contribute effectively towards monitoring visually impaired people in real-world scenarios where assistance is essential.

\subsubsection{Critical Analysis of the Research Item}

The robustness of this work lies in its hybrid nature, borrowing the advantages of recurrent and autoencoder-based temporal analysis and CNN-based feature extraction. Its use with real-time sensing using IoT devices enhances over static image processing to offer real-time situational awareness. Adaptive Bilateral Filtering (ABF) usage also enhances image quality, leading to increased recognition accuracy. Nevertheless, the model's high computational cost restricts its portability to low-capacity platforms like smartphones. Relying on a single dataset (UR Fall Detection) in an effort to reduce generalization to more varied environments is another restriction. In addition, the lack of user trials involving visually impaired users ensures that real-world usability, latency, and energy efficiency of the system are not realized. Although the findings are encouraging for smart indoor safety, accessibility and scalability continue to be key challenges. 
...
\subsubsection{Critical Analysis of the Research Item}  
A major strength of this work is its successful adaptation of a large model (Florence-2) for edge deployment. The system does fine-tuning and quantization to achieve computational efficiency and contextual precision. Value is added beyond pointable object recognition in the form of the distance-aware detection and the personalized TTS with the possibility of customization. Even so, there are limitations. The requirement for NVIDIA Jetson hardware can limit accessibility with respect to smartphone-only implementations, and the system's latency of 5 frames, although comparative low, can potentially compromise responsiveness in rapidly changing scenes. In addition, the analysis focuses heavily on accuracy and efficiency but sheds limited light on the longer-term user studies and the robustness in varied real-world conditions (e.g., low illumination, high-population indoor scenes, outdoor variations).

\subsubsection{Relationship to the Proposed Research Work}  
VocalEyes \cite{ref:chavan2024vocaleyes} is a major reference for our work since it illustrates the great potential of tuning large AI models to be used in real-time assistive applications. The system works on top of edge hardware but we are striving to provide similar features, namely contextual object detection and real-time feedback, with the help of a smartphone camera only, to make the system available at a larger scale. The use of distance measurement and adaptive voice feedback in VocalEyes is very much in line with the intent of offering easy and lovable navigation support to the user. On the other hand, the problems of hardware dependence and latency raised in the research pointed out that developing lightweight, mobile-optimized solutions that are independent of external processing devices becomes even more important, and this is exactly where our focus of research is.
...
\subsubsection{Summary of Research Item}  
Pfitzer et al. \cite{ref:pfitzer2025mr} present MR.NAVI, a mixed-reality navigation system designed to enhance spatial awareness and mobility for visually impaired individuals. The system presents a complete navigation assistant by the combination of computer vision, natural language processing, and spatial audio feedback. Operated on the Microsoft HoloLens2, it integrates object detection, depth estimation, obstacle avoidance as well as public transit navigation under a single framework. MR.NAVI is operating at the same time as an Electronic Orientation Aid (EOA), Electronic Travel Aid (ETA), and Position Locator Device (PLD), filling the space that existing solutions often leave behind when they only provide one or two of these functionalities. The application provides audio and visual feedback of scene descriptions, collision avoidance guidance, and navigation directions, while voice commands are the main user interface for the users. User studies demonstrated that MR.NAVI improved participants’ confidence in navigating unfamiliar environments, though limitations in spatial audio precision and environmental adaptability were noted.

\subsubsection{Critical Analysis of the Research Item}  
The primary strength of MR.NAVI lies in its comprehensive integration of EOA, ETA, and PLD features into a single mixed-reality platform, which many competing systems fail to achieve. The utilization of multimodal feedback in the form of spatial audio, visual indication, and natural language response makes the system capable of use on a variety of visual impairments. Moreover, using available consumer hardware such as HoloLens2 shows pragmatism over very special-purpose custom-built devices. Yet, the dependency on HoloLens2 generates accessibility limitations since the device is expensive and less mobile than smartphone-based solutions. Dependence on high Wi-Fi connectivity and a server on the PC for processing-intensive tasks brings into question real-world usability and scalability. User studies, as enlightening as they were, used blindfolded non-sighted participants and not visually impaired ones, which compromises ecological validity. In addition, the outdoor performance of the system was troubled by lighting changes and sensor limitations, revealing the vulnerabilities of robustness in real-world settings.

\subsubsection{Relationship to the Proposed Research Work}  
The findings of MR.NAVI \cite{ref:pfitzer2025mr} are significant to our target smartphone-based navigation system, Sighteo. Although MR.NAVI demonstrates the capabilities of integrating various assistive technologies in a single framework, its reliance on mixed-reality hardware makes it less practical for mass adoption. By contrast, our project aims at replicating the same functionalities with the least and simplest hardware consisting only of a smartphone camera, thus it is low-cost and accessible. The focus in MR.NAVI on natural feedback strategies (spatial audio and natural language) aligns with our goal to offer constant, natural guidance by audio and touch feedback. But while MR.NAVI was mostly tested with simulated environments and necessitated server support, our system will focus on light, mobile-optimized computation to support real-time guidance without the need for external devices. In essence, MR.NAVI demonstrates the advantages of multimodal, AI-driven navigation but also underscores the importance of portability, affordability, and independence from expensive hardware, principles central to our proposed work.
...
This work presents a hybrid AI approach combining the strengths of convolutional neural networks, transfer learning, and a cooperative particle-based algorithm to enhance object classification performance. The authors blend these techniques in the pursuit of higher recognition accuracy for more reliable assistive support in users. This study shows how layered improvements in model design can translate into more dependable functionalities in the real world.
...
\subsubsection{Critical Analysis of the Research Item}  
The primary contribution of this research is the hybrid framework, combining CNNs and transfer learning with particle competition/cooperation in an effort to enhance classification accuracy. This combination addresses typical issues like small training sets and poor generalization in real environments. Transfer learning reduces the necessity of large budgets for training, hence making the solution more deployable. Nevertheless, the use of computationally expensive deep models within the system might restrict its portability and real-time suitability on devices like smartphones. The work focuses primarily on classification and detection of objects and does not include feedback information for how end-users are receiving feedback when they are navigating. The research is also not enabled with extensive real-world usability testing involving visually impaired participants, bringing its use in real-world environments with changing contexts into doubt.


\subsubsection{Relationship to the Proposed Research Work}  
For our project, Breve and Fischer's article \cite{ref:breve2020visually} is an informative resource regarding how transfer learning and CNNs can be used to improve object detection accuracy in assistive systems. Their approach shows that the power of several algorithms being able to increase performance can be harnessed, which also aligns with our goal of creating a trustworthy smartphone-based navigation assist. But whereas theirs is directly aimed at recognition accuracy, our solution builds on theirs by extending it with real-time navigation hints, collision detection, and multimodal feedback (auditory and haptic) for independent indoor mobility. By aiming at light, smartphone-based solutions, our system aims to circumvent the computational resource constraints of their solution while leveraging its advantage in AI-based object detection.
...
This work presents an interactive indoor navigation system based on an AI-driven chatbot, operating with natural language processing and a network of environment-optimized nodes to provide users with voice-based, step-by-step instructions. In this instance, the approach is being demonstrated by showing that familiar dialogue-based interactions may make navigation more intuitive and minimize the learning curve for those with a visual impairment.

\subsubsection{Summary of the Research Item}

  Thandu and Rathinam \cite{ref:thandu2024enhancing} also introduced an AI indoor navigation system that leveraged Natural Language Processing (NLP) and VEO (Virtual Environment Optimization)-optimized nodes in order to guide visually challenged people through an interactive chat-like interface. The system automatically guides people in indoor spaces based on voice queries and gives directions in a conversational mode. The chatbot's engagement is with a cluster of VEO-optimized nodes that are technically affixed to the environment and thus present an avenue for dynamic path generation and guidance of users depending on the different contexts. The chatbot is empowered to discern user intent by means of NLP algorithms and subsequently offer feedback that is of great value and awareness to the situation. The entire approach is solely aimed at making indoor navigation more intuitive and interactive by means of dialogue-based support that in turn would lessen the barrier between accessibility and user engagement. The findings from the experimental study showed that this system not only had a very high response accuracy rate but also decreased disorientation of users when compared to the case of static voice guidance techniques.

\subsubsection{Critical Analysis of the Research Item}

The main merit of the work of Thandu and Rathinam is the hybridization of conversational AI with the already present indoor navigation in real-time. The system using NLP provides an apt and more human-like way for the visually impaired people to ask for directions as well as get guidance. The feature of having VEO-optimized nodes incorporated in the system allows the computing of the path to be done faster and the environment to be more adaptable. On the negative, the model only browses through communication and route response whereas it can also serve low-level perception tasks such as obstacle detection and environmental sensing. Due to this fact, the use of the device in such environments where visual feedback or real-time avoidance of hazards is a requirement becomes less suitable. Moreover, the researchers didn't state anything regarding latency, fault tolerance, or noise robustness performance indexes which are all essential for the assistive technologies. Another point indicating a tech reliance on mapping from preset nodes is the low scalability for bigger or unmapped indoor spaces.

\subsubsection{Relationship of the Proposed Research Work}

Thandu and Rathinam's work \cite{ref:thandu2024enhancing} is essential for Sighteo because it extends the area of assistive navigation to include AI-driven communication that is truly interactive. Although the Sighteo system under consideration only detects obstacles using the smartphone camera and creates spatial awareness, the augmentation of a conversational interface for guidance is a possible future move. The findings from the current research indicate that feedback systems that are adaptable, user-centered are essential in making navigation compliant. The Sighteo system not only accepts this but also proposes to replace map-based navigation with real-time visual perception and to provide feedback in two forms (audio and haptic) thus offering both interactive and perceptual support without the need for an already defined environment.
...
This article proposes an intelligent assistive system that incorporates object detection with ultrasonic sensing for safe navigation by visually impaired people. Merging the visual cues with accurate distance measurements, the system delivers timely audio warnings to users about obstacles in proximity. This layered sensing helps to make navigation more dependable and responsive.
...
Kral et al. \cite{ref:kral2025low} present a pair of smart glasses. Their technology merges text recognition, obstacle detection, and distant help into one economical packing. The setup is organized around a Raspberry Pi Zero 2W, with an Arducam IMX519 autofocus camera, and operated through tactile button interfaces for users' convenience. On the spot audio feedback notifies users of the presence of objects, and recognized text is read to the user making the user alert in the situation. The gadget is wanted at a rehabilitation center, where the testing results showed very high accuracy in text recognition, and controlled settings gave better navigation support. The authors have also compared their approach with that of commercial products and have claimed that their solution is more attractive and less bulky. They point out limitations in processing speed, hardware size, and the future of their research includes investigating haptic feedback and AI-driven scene analysis.

\subsubsection{Critical Analysis of the Research Item}

A key strength of this study is the integration of low-cost components like the Pi Zero and Arducam camera to provide multiple assistive functions like text recognition, obstacle detection, and remote support into a single wearable device. Its testing was done at a rehabilitation center, which adds to its credibility. However, its reliance on audio feedback alone limits usability in noisy environments, and the bulky, power-hungry hardware may affect comfort and battery life. The system also faces latency issues due to limited processing power. The authors note that future work should focus on improving speed, miniaturization, and broader field testing with visually impaired users.
...
Zhang et al. \cite{ref:zhang2024improved} & YOLOv5 + GhostNet + BiFPN with depth camera; audio output & 42.4\% model-size reduction; +1.2\% recall vs. baseline & Requires depth sensor; controlled setting \\ \hline
...
Al Duhayyim \cite{ref:al2025ensemble} & EDLES-SIAM: ABF + ResNet50 features + BiLSTM/SSAE/DNN ensemble with IoT & 99.25\% (UR Fall Detection) for HAR/fall detection & High compute; IoT infra required; no user study \\ \hline
...
The literature review highlights that while AI-based navigation technologies have progressed significantly, each approach presents trade-offs. Research studies showcase strong accuracy and innovative techniques, but often rely on expensive hardware or lack real-world testing. Smartphone-based works improve accessibility but are mostly restricted to object recognition without full navigation support. In conclusion, despite the extensive research on the topic, a portable, affordable, and a truly navigational system has not been developed as of now. Our proposed system aims to directly address these challenge by relying solely on smartphone cameras and providing real-time feedback through audio and haptic cues.

\chapter{Software Requirement Specifications}
A descriptive account of software requirements for the proposed AI-based navigation system, Sighteo, has been presented in this chapter. The functional and non-functional requirements, key features, and quality attributes of the system are outlined in chapter. It documents how the system will operate, the services it will provide, and the constraints under which it must function. Furthermore, this chapter makes sure that the requirements are precise, testable, and serve as a solid base-line foundation for the system design and future development. The details of specifications documented in this chapter provide a comprehensive understanding of what the software must achieve to effectively address the needs of its users.

\section{List of Features}
 Our mobile app aims to revolutionize navigation for visually challenged users by providing the following features:

\begin{itemize}
    \item Smartphone camera can be used by the users to receive real-time obstacle detection.
...
\begin{itemize}
    ...
    \item \textbf{Scalability:} System would be extensible to support more users in the future.
\end{itemize}

\section{Non-Functional Requirements}
\begin{itemize}
    \item The system shall navigate in a reliable manner without unexpected failures to provide navigation at all times.
    \item The system shall provide data integrity, preventing data loss or corruption during in-application communication.
    \item The system shall have an intuitive and user-friendly interface, with should be easy for visually impaired individuals to navigate.
    \item The system shall provide exact and real-time navigation guidance and detection of objects.
...
\begin{figure} [!h]
    \centering
\includegraphics[height=0.55\textwidth]{Figures/GUI2.png}
...
\begin{figure} [!h]
    \centering
\includegraphics[height=0.55\textwidth]{Figures/GUI1.png}
...
\begin{figure} [!h]
    \centering
\includegraphics[height=0.54\textwidth]{Figures/GUI3.png}
    \caption{Navigation Page\\}
    \label{fig:5}
\end{figure}
\pagebreak
\section{Database Design}
Our database schema is modular in design. The user profile information is stored in the user table, while the contact information has been divided for ease into the contact details table. The login credentials are securely kept by the authentication table for proper account management. Lastly, the user feedback and ratings related to the app are kept in the feedback table. In designing this schema, our intention was to keep it simple, avoid redundancy so each tableshould play a specific, dedicated role.
\subsection{ER Diagram}

    \begin{figure} [!h]
        \centering
    \includegraphics[height=0.45\textwidth]{Figures/erd.png}
...
Identification of the potential risks and uncertainties likely to impact project progress is a very crucial activity in the process of project planning. In order to ensure that the development process runs smoothly and the project is delivered successfully, risk analysis of our system considers many aspects with the purpose of efficiently counteracting possible problems.

\begin{itemize}
    \item \textbf{Hardware Failures:} Although the application has low hardware requirements, some unexpected device failures (like camera failure, malfunctioning speakers, or malfunctioning vibration motor) may impede service availability. Routine maintenance of the devices and fallback notifications are recommended to counteract this risk so that users are always in time notified in case essential hardware is not available.
    
    \item \textbf{Software Compatibility:} The application requires recent versions of mobile operating systems (Android/iOS). Camera usage, audio playing, or background services may be incompatible due to unsupported/older versions. Regular updates of apps and operating systems forced on users can decrease this risk.
...
This chapter sets out the general approach informing the design of our assistive navigation system, Sighteo. It describes how the application is structured, how the core models are chosen and integrated, and how user feedback informed every design decision. The following sections outline the methodology, tools, and ethical considerations guiding the project, along with the long-term directions beyond the initial indoor navigation focus.
\section{Overview of the Proposed Approach}
Creation of a mobile application that aids visually impaired people in the navigation of their daily environment with the help of a smartphone's camera alone, rather than earlier solutions that were reliant on external sensors like LiDAR or GPS, is our primary goal. Our system focuses on accessibility and affordability by making use of computer vision (CV) models running on the backend. Real-time video frames, captured by the smartphone camera, are sent to the backend for processing. The backend processes the input frame by making use of object detection and vision-language models, then sends audio and haptic feedback to the user. 
...
\begin{enumerate}
    ...
    \item \textbf{System Architecture Design:} Mobile frontend developed using React Native, ensuring compatibility across both iOS and Android. Backend implemented in Python for AI inference models.
...
\begin{itemize}
    \item \textbf{Frontend:} React Native with TypeScript  
...
Particular attention will also be given to accessibility since the target members will be visually impaired individuals. Adherence to the level of provision of the WCAG standards, protection of the privacy of the taken photographs, and omission of unnecessary user inputs belong to this. In order to ensure ethical considerations and inclusivity, the system will be tested with the involvement of real users.

\section{Future Directions}
Although the initial focus is on indoor navigation in controlled environments, the methodology helps us to progressively extend the scope of the project in the future. The project roadmap envisions:
\begin{itemize}
    \item Semi-controlled indoor environments (e.g., public buildings with moderate variations).  
    \item Dynamic indoor environments with moving objects and unpredictable layouts.  
    \item Outdoor navigation as the final extension, incorporating distance-aware detection in open, unconstrained settings.  
\end{itemize}

\section{Conclusion}
In short, the methodology blends AI object detection and real-time feedback into a mobile application. It begins with the selection of a suitable base model from out of YOLOv8, SSD and RetinaNet. After selection, we'll move to fine tuning the model to the specific needs of the vusially impaired individuals. Scalability, affordability, and accessibility are offset in the project by the utilization of lightweight on-device computation managed with help of Python and React Native. Agile development model allows for iterative fine-tuning. Aim is to increase inclusivity and trust in the visually challenged community. This will be ensured through ethical guidelines.
...
Sighteo's system is organised into a set of interconnected modules that work together to deliver the final objectives of the application. Each module handles a specific part of the workflow. Following describes the compositional structure of the system:
...
This module provides users with an option report an any issue that they encounter using the application. A button will be provided on the homepage for it. This will provide endless opportunities for application refinement.
...
The training phase resolves around preparing the dataset collected for the detection model. The dataset is annotated with object classes for indoor navigation. The AI model will be trained and fine-tuned to provide a balance between detection and inference speed. Experimental comparisons would be conducted among different AI models and their respective variants to make sure that the chosen model is the most stable and reliable. The chosen model is then trained and integrated into the application to provide the best navigational output as possible.

\section{Design Considerations}
The following are the key concerns that should be identified prior to the finalization of the Sighteo system design: These help in pointing out various technical limitations, user experience expectations, and environmental issues that could control how an app would function in real-world situations. By analyzing such concerns at an early stage, the design should remain practical and scalable, and meet the needs of visually impaired users by ensuring that each and every design decision supports the requirements for safety, reliability, and consistency in navigation.
...
\item \textbf{Operating systems:} The application would be compatible with Android and iOS.
...
\item \textbf{Standards compliance:} The design must show compliance with accessibility standards for visually impaired individuals. It should also obey platform specific guidelines like Google play and Apple app store requirements.
...
\item \textbf{Interface/protocol requirements:} The user interface must be simple ensuring easy transition between the login, main page, and feedback page. Communication protocols between app modules (camera feed, feedback delivery, etc) should work efficiently in real time.

    \item \textbf{Data repository and distribution requirements:} User account information must be safely stored in a password-protected database. Data distribution, such as model updates, must go via secure channels to prevent security issues.
    \item \textbf{Security requirements (or other such regulations):} The system must protect sensitive information, including login credentials, from unauthorized access. Data in storage and transmission should be encrypted to avoid tampering.
...
\begin{figure} [!h]
    ...
    \includegraphics[height=0.60\textwidth]{Figures/highLevel.png}
        ...
        \label{fig:7}
    \end{figure}
The architecture of the system is structured into three interconnected layers: presentation, logic, and data. The presentation layer consists of a mobile application that is the main point of interaction between the user and the system. It uses the camera to capture real-time video frames and delivers processed results back to the user as audio or vibration feedback, making it easier for visually impaired individuals to respond to their surroundings. The logic layer is the processing hub of the system. At this level, the video frames are passed through the object detection model, which detects obstacles in the environment, and the navigation logic, which determines how the user can avoid these obstacles and move safely. Lastly, the data layer stores vital information such as user details, credentials, and feedback. This ensures secure access to the system while allowing continuous improvement based on user experiences. Together, these layers provide a smooth workflow for reliable and real-time navigation.
\pagebreak
...
\includegraphics[height=0.38\textwidth]{Figures/Component.png}
    ...
    \label{fig:8}
\end{figure}
Sighteo's subsystem architecture is organized into interconnected modules that work alongside each other to provide navigation cues. The camera module captures real-time video, which is sent to object detection module as video frames for obstacle identification. This information flows into the navigation module, which calculates safe paths and returns instructions to the user through audio and haptic feedback. The User management module handles authentication and stores sensitive data in the database, while the feedback module stores the experiences shared by the users, which are then used for analysis. Together, these subsystems ensure reliable, responsive, and user-centered navigation support.

\section{Architectural Strategies}

The architectural strategies for the Sighteo app focus on providing dependable real-time assistance to visually impaired users while establishing cross-platform compatibility, system scalability, and strong security measures. The mobile app is developed in \textbf{React Native}, while all the backend will be built in python. This division makes sure that the application runs light and remains responsive on mobile devices, while the backend efficiently manages AI workloads and heavy computations.

\subsection{Tools and Software}
This section describes the technical basis on which Sighteo can function reliably in real time. It summarizes the major software components, hardware requirements, flow of communication, and safety features that are integrated into the overall design.
\subsubsection{Mobile Application:}
\begin{itemize}
  \item Developed using \textbf{React Native}, enabling simultaneous compatibility with both iOS and Android.
  \item Integration with native accessibility APIs such as iOS VoiceOver and Android TalkBack.
...
\begin{itemize}
  ...
  \item Machine Learning inference powered by \textbf{TensorFlow} or \textbf{PyTorch}.
  \item Middleware layer for request handling and load balancing.
\end{itemize}

\subsubsection{Communication Protocols:}
\begin{itemize}
  \item Lightweight payloads (JSON/protobuf) carry detection outputs such as object type, relative distance, and confidence scores.
\end{itemize}
...
\includegraphics[height=0.5\textwidth]{Figures/classDiagram.png}
    \caption{Class Diagram (Tentative)\\}
    \label{fig:9}
\end{figure}

\section{Policies and Tactics}

This section describes the design policies and strategies used during Sighteo development. These choices, though not affecting the system's top-level architecture significantly, affect implementation details, coding style, testing, and maintainability in the long term.

\subsection{Conventions and Coding Guidelines}
For Sighteo development, coding standards shall be strictly adhered to ensure readability, consistency, and maintainability. The mobile app will be built using React Native, whereas backend and integration of models will be done using Python. Proper indentation, module-based coding standards, and explanatory comments shall be used throughout the codebase. This makes it easy for future developers to comprehend and extend the project.

\subsection{Coding Environment}
The primary Integrated Development Environment (IDE) will be Visual Studio Code due to its extensive support for React Native and Python. It has built-in debugging tools, syntax hints, and extensions for version control like github. Furthermore, to manage Python environments during model training and experimentation, Anaconda will be used.  
...
Sighteo is extensible. Its code is written in a modular way such that future developers can easily plug in new object detection models, other sensors, or different feedback mechanisms. This makes it possible for the system to continue evolving with technological progress and user requirements.  

\subsection{Engineering Trade-offs}
A major design trade-off in Sighteo is the selection of YOLOv8 over more recent versions such as YOLOv10. Although more recent versions could potentially provide slight improvements, YOLOv8 has been chosen because it has an established balance of being fast, accurate, and stable, and hence is more reliable for real-time navigation applications on handheld devices.

\subsection{Requirements Traceability}
To ensure that all requirements are addressed, a mapping between functional requirements and implemented modules will be maintained. This traceability will help verify that user needs and system goals are fully met during development and testing.  

\subsection{Deliverables and Build Process}
The system deliverables will include the mobile application for both android and iOS, and documentation of the trained YOLOv8 model. The build process will involve compiling React Native code, linking it with the backend services. 

\section{Conclusion}
The architecture, subsystem interactions, design principles, and technical strategies that constitute the Sighteo system were introduced. Particular details on how the application processes data, manages user interactions, integrates the detection model, and presents real-time feedback have been provided. This chapter has established that the system is based on a scalable, maintainable, and user-centered foundation by identifying assumptions, constraints, and the ways in which it has been developed. It is these types of design decisions that provide the backbone for the implementation work continuing in successive chapters.

\chapter{Implementation and Test Cases}
Work done in terms of implementation of the application has been described in this chapter, including technical details which are not discussed in the high and low-level design.
\section{Implementation}

The implementation phase contained two main components. Firstly, a methodical model selection and fine-tuning process was undertaken. Secondly, the finalized model was integrated in an intuitive and functional React Native mobile application for real time navigation assistance.
...
Three object detection architectures (YOLOv8, SSD, and RetinaNet) of the present time were evaluated. Their selection was based on their documented performance in real time detection tasks. Each of the architectures was examined for their detection accuracy, inference speed, computational complexity and deployment feasibility in mobile devices. Particular focus was placed on achieving real time frame rates suitable for walking scenarios, as the latency directly affects user safety. Preliminary benchmarking showed that YOLOv8 had the most balanced trade-off between detection accuracy and inference speed.

\subsubsection{Dataset Selection and Preparation}

Publicly available datasets, namely COCO and Open Images, were used as the foundation for initial experimentation. Given that these datasets cover a wide range of object categories, a filtering process was added to keep the indoor relevant classes, such as chairs, tables, doors, stairs and people. Data augmentation techniques (resizing, rotation, brightness and random cropping) were used to mimic the indoor variability of the real world (e.g. lighting fluctuations, cluttered environments).

\subsubsection{Evaluation Metrics}

In order to compare models in an objective way the following metrics were used:

\begin{itemize}
    \item \textbf{Mean Average Precision (mAP):} Mmeasures the quality of detection for the different classes of objects.
...
After optimizing the model, the trained model (YOLOv8) was integrated into a React Native mobile application designed for visually impaired people.

\subsubsection{Model Deployment}

The trained model was converted to a mobile compatible format to make it easier to perform on-device inference. This conversion removed the need for cloud processing and therefore reduced the latency and increased the reliability in environments with limited connectivity.

\subsubsection{Real-Time Detection Pipeline}

The mobile application captured the frames from the camera continuously and these frames were sent to the embedded object detection model. Detected objects were processed to determine relative positioning and possible collision risk. A lightweight and rule-based logic layer gave priority to the obstacles that were most immediately dangerous.

\subsubsection{Feedback Translation Layer}

Detection outputs were converted into actionable advice through a feedback module. Audio prompts provided directional information, while haptic vibration patterns indicated proximity and urgency. This multimodal method ensured accessibility without causing cognitive overload.

\subsubsection{End-to-End Testing}

Integrated system testing was carried out in controlled indoor settings, comprising of corridors and rooms with obstacles. Performance measured from detection reliability, response latency and clarity of user feedback. Observations from testing led to iterative refinements in both the detection model and the mobile interface.

\section{Test Case Design and Description}

The test cases designed for Sighteo were aimed at testing the functional operation of the mobile navigation system in controlled indoor environments. Because the project is aimed at helping visually impaired users with obstacle detection in real time and guidance, the test cases were built around the principal user flows and system responses of the application.

All tests were run on Android mobile devices with operative camera access, speaker output, vibratory support, and adequate processing capability to run the integrated object detection model. The tests were conducted in an indoor environment, such as corridors, classrooms, rooms, and hallways, under normal lighting conditions, unless otherwise specified. For the tests related to navigation, it was assumed that the application had been installed properly, the necessary permissions had been granted, and that the user had already activated the navigation module.
...
\textbf{Test Date:} & 11/03/2026 & \textbf{Use Case Reference(s):} & Register User \\ \hline
...
\textbf{Test Case Version:} & 1.0 & \textbf{Reviewed By:} & Farooq Haider \\ \hline
\textbf{Test Date:} & 11/03/2026 & \textbf{Use Case Reference(s):} & Login User \\ \hline
\textbf{Revision History:} & \multicolumn{3}{p{10.2cm}|}{None} \\ \hline
...
\textbf{Test Case Version:} & 1.0 & \textbf{Reviewed By:} & Farooq Haider \\ \hline
\textbf{Test Date:} & 11/03/2026 & \textbf{Use Case Reference(s):} & Navigate User \\ \hline
\textbf{Revision History:} & \multicolumn{3}{p{10.2cm}|}{None} \\ \hline
...
This chapter elaborates on the testing methodology used in order to validate the implemented components in the Sighteo system. A set of functional test cases were developed to evaluate key features such as user authentication, camera initialization, real-time obstacle detection, and combining audio and haptic guidance for navigation assistance. Each of the test cases was performed under controlled indoor conditions to make sure that the system would react to expected user interactions and environmental inputs. The results of the tests carried out show that the implemented modules are functioning as per the intended requirements and meet the functional requirements specified in the Software Requirements Specification. Test metrics and the Requirements Traceability Matrix further support the fact that all the major features of the system were tested through systematic testing. As a result of the testing process, it is possible to affirm that the developed prototype works reliably within the scope defined for the project.



\chapter{User Manual}

The chapter is a detailed user guide to the Sighteo mobile application. The manual should serve as a guide to the user on the installation, access and usage of the main features of the system and as such, help the user operate it efficiently and exploit the navigation assistance capabilities effectively.
...
\item Put the device at the front of you so that the camera is able to detect the environment around.
...
\section{Quantitative Comparison of Detection Accuracy}
...
\section{Class-wise Performance on COCO}
...
\section{Discussion and Model Selection for the Navigation System}
...
In the context of a smartphone-based assistive navigation app for visually impaired users, latency, energy usage, and model size are as important as raw accuracy. A slightly-less-accurate model that runs reliably and fast on a mobile CPU is more useful in practice than a very accurate model responding several seconds late.

Based on experimental evidence from this chapter, YOLOv8 is picked for our prototype navigation system as the main detection backbone. In the subsequent chapter, we finetune YOLOv8 on our indoor dataset and integrate it into the pipeline of the mobile application, focusing on enhancing recall for safety-critical obstacle classes while preserving the real-time performance seen in this evaluation.


\section{Summary of Comparative Analysis}
This section summarizes the behavior of YOLOv8, SSD, and RetinaNet in terms of accuracy, speed, resource usage, and class-level precision in structured tables, hence providing readers with to-the-point insights regarding the trade-offs emerging during testing. It is an attempt to make this comparison easy to navigate, especially for diverse metrics influencing real-world model suitability.

\subsection{Comprehensive Model Comparison}
This work presents a summary of the performance of the models discussed above: YOLOv8, SSD, and RetinaNet. All results are tabulated to make it clear and easy to refer to; most key metrics have been brought out for each model in terms of accuracy, speed, and efficiency. The table format allows readers to rapidly scan and compare the performance of the models in a succinct and structured manner.
\begin{table}[h!]
\centering
...
\subsection{Class-wise Average Precision}
...
\section{Conclusion}
This chapter provided a thorough comparison between models-YOLOv8, SSD, and RetinaNet-using a common evaluation pipeline. The various differences in accuracy, speed, and efficiency were evident, with each model having a different strength. YOLOv8 had superior real-time performance and overall balance, so it was selected as the preferred model for further experimentation. SSD had moderate results, while RetinaNet achieved the highest accuracy with a significant computational cost. This helps in effective model choice for indoor object detection.

\chapter{Conclusions}
Our final year project was to develop an obstacle-detecting assistive mobile application for visually impaired persons with real-time object detection using a smartphone's camera. The project was divided into two phases, namely FYP-1 and FYP-2, where the first one focused on the research and planning, and the second focused on the implementation and evaluation. The scope defined was accomplished and covered all the planned objectives of research, analysis, model selection and application development. Also, our focus was restricted to indoor environments only because the extension of the system to the outdoors would require significantly larger datasets and greater computational resources.

Our group laid the foundation for the project by conducting research, data analysis, and designing methodology throughout the first stage, i.e. FYP-I, of the Final Year Project. The major deliverables were:
\begin{itemize}
    \item \textbf{Comprehensive Literature Review:}
    We conducted an extensive review of 15 research articles. This made us understand the existing solutions, identify technological gaps, and subsequently define the scope for our project in a concrete way.
...
The second phase involved the \textbf{development and implementation} of the solution - an assistive application targeted at visually impaired people. It enabled the detection of obstacles by means of a phone's camera to guide users without the use of any other additional hardware. The main goals that the second phase, FYP-2, focussed on are:
...
\begin{itemize}
    ...
    \item \textbf{Testing and Validation:}
     ...
     Engaged visually impaired users in testing the application analyzing its practical usability and effectiveness.
     \item \textbf{Feedback Analysis:}
     Gathered and assessed users' suggestions to identify areas that could be further improved and refined the final application.
\end{itemize}

FYP-1 built the conceptual and technical foundations of our work, while FYP-2 transformed those into a functional application. Put together, these two phases provided an intelligent, accessible, and reliable assistive technology for the visually impaired.
